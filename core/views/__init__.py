from core.views.dashboard import *
from core.views.getModels import *
from core.views.exam import *
from core.views.details_exam import *
from core.views.edit_exam import *
from core.views.gestion_exam import *
from core.views.student import *
from core.views.settings import *
from core.views.EditModels import *
from core.views.editStudent import *
from core.views.profile_admin import *
from core.views.submitExam import *


#  -----------------------------views etudiant ----------------------------------- #

